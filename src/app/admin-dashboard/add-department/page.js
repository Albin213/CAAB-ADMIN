...
function AddDepartment() {
  ...
  const [departmentTableRenderToggle, setDepartmentTableRenderToggle] = useState(false);
  const [selectedDepartmentId, setSelectedDepartmentId] = useState(null);
  const [isDeleteDepartmentPopupOpen, setIsDeleteDepartmentPopupOpen] = useState(false);
  const [isEditDepartmentPopupOpen, setIsEditDepartmentPopupOpen] = useState(false);

  // Handle Input Changes
  function handleOnChange(event) {
    const { name, value } = event.target;
    setDepartmentData((prev) => ({ ...prev, [name]: value }));
    setErrors((prev) => ({ ...prev, [name]: false })); // Clear errors when user types
  }
  ...
  async function handleAddDepartment(event) {
    ...
    try {
      ...
      setTimeout(() => {
        setDepartmentData({ departmentName: "", departmentType: "", governmentType: "" });
        setIsBtnDisabled(false);
    ...
    <div className="w-full flex flex-col gap-10">
      ...
      <div className="pb-10 flex gap-3 xl:gap-6 border-b-[1px] border-[#C2C6D4]">
        <input
          ...
          placeholder="Name of Department"
          className={`w-[150px] lg:w-[180px] xl:w-[261px] h-10 px-4 py-2 text-[11px] lg:text-sm font-normal text-[#404753] bg-white rounded-lg outline-none border-[1px] border-[#707784] focus:border-[#782A99] focus:border-2 placeholder:text-[#404753] ${
            errors.departmentName && "border-red-500 border-2"
          }`}
          name="departmentName"
          value={departmentData.departmentName}
          onChange={handleOnChange}
        ...
        <select
          className={`w-[150px] lg:w-[180px] xl:w-[261px] h-10 px-4 py-2 text-[11px] lg:text-sm font-normal text-[#404753] bg-white rounded-lg outline-none border-[1px] border-[#707784] focus:border-[#782A99] focus:border-2 ${
            errors.departmentType && "border-red-500 border-2"
          }`}
          name="departmentType"
          value={departmentData.departmentType}
          onChange={handleOnChange}
          defaultValue=""
        >
          <option value="" disabled>Department Type</option>
          <option value="EMP">EMP</option>
          <option value="NON EMP">NON EMP</option>
        </select>
        ...
        <select
          className={`w-[150px] lg:w-[180px] xl:w-[261px] h-10 px-4 py-2 text-[11px] lg:text-sm font-normal text-[#404753] bg-white rounded-lg outline-none border-[1px] border-[#707784] focus:border-[#782A99] focus:border-2 ${
            errors.governmentType && "border-red-500 border-2"
          }`}
          name="governmentType"
          value={departmentData.governmentType}
          onChange={handleOnChange}
          defaultValue=""
        >
          <option value="" disabled>Government Type</option>
          <option value="Central">Central Government</option>
          <option value="State">State Government</option>
        </select>
        ...
        <button
          ...
          disabled={isBtnDisabled}
        >
          {isBtnDisabled ? "Adding..." : "ADD DEPARTMENT"}
        </button>
      </div>
